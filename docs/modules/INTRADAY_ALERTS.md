...
- ETF 历史趋势、前高回撤和前低反弹必须读取估值报告中的可比序列口径；优先使用 `fund_nav.adj_nav` 复权净值。盘中实时价仍来自 QMT，但若报告提供 `realtime_price_multiplier`，作战地图应把实时交易价换算到复权净值口径后再更新当前回撤/反弹位置。
- ETF 若只能使用未复权交易价格代理，作战地图必须在 tooltip 中提示“未复权价格代理，遇到分红除权可能失真”，不得把该口径当作高可信回撤。
- 实时当前位置：黑色竖线。
...
- 仓位差距：显示当前仓位、目标区间和偏离。
- Tooltip 应支持较长阅读时间，用于解释风控位、右侧确认位、实时估值区间、报告基准区间、历史比较口径、回撤/反弹和仓位偏离；鼠标移出图示区域后再消失。
