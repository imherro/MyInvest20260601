...
生成类似券商晨报的综合报告，包含重大新闻、策略精要、市场分析、重点方向、核心观点和风险提示。用户说“盘前分析”“盘前策略”“策略简报”“今日晨报”“市场策略”时，默认使用本模块。

### 08 盘前执行检查模块

确认已有研究和操作建议是否可以进入执行，只做执行门禁和盘中监控清单。
...
- 确定 ETF 研究模块先建立观察池，再逐只生成 ETF 档案；ETF 档案必须引用最新市场仓位报告、最新主线研究报告和主线双评级。进入观察池不等于可买清单。
...
- 确定新增盘前策略简报模块：`docs/modules/STRATEGY_BRIEFING.md`、`templates/strategy_briefing_template.md`、`templates/strategy_briefing_template.json` 和 `research/briefings/`。
- 确定用户说“盘前分析”“盘前策略”“策略简报”“今日晨报”“市场策略”时，默认生成盘前策略简报，而不是只生成盘前执行检查。
- 盘前策略简报应提供类似券商晨报的信息结构：重大新闻、策略精要、市场分析、重点方向、核心观点、今日观察清单、风险提示和模块 handoff。
- 盘前策略简报可以给方向、观点和条件，但不能绕过 `ACTION_PLAN` 直接生成买入、加仓、减仓或卖出动作。
- 确定 yfinance 和 FRED 纳入项目数据源体系：yfinance 用于海外股票、海外 ETF、海外指数、ADR 和跨市场行情补充；FRED 用于美国利率、通胀、就业、金融条件和全球宏观时间序列。相关模块已通知：市场仓位、盘前策略简报、ETF 研究和个股研究可在需要时调用；盘中实时 A 股行情仍以 QMT 为主。
