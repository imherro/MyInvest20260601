...
def build_report(
    ...
    *,
    ...
    return {
        ...
        "db": rel_path(path),
        "filters": {"code": code, "bucket": bucket, "action_type": action_type, "limit": limit},
        "summary": {
        ...
        "market_history": query_market_history(path, limit=limit) if path.exists() else [],
        "valuation_history": query_valuation_history(path, code) if path.exists() and code else [],
        "position_history": positions[-limit:] if limit else positions,
        "action_history": query_action_history(path, code=code, action_type=action_type, limit=limit) if path.exists() else [],
        "boundary": {
            "ratio_only": True,
...
def write_outputs(report: dict[str, Any], out_dir: Path, fmt: str) -> dict[str, str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(args.db, code=args.code, bucket=args.bucket, action_type=args.action_type, limit=args.limit)
    created = write_outputs(report, ROOT / args.out_dir, args.format)
