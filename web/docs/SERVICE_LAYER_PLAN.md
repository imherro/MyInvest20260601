...
28. Migrate action plan generation in a future phase only after target allocation promotion remains stable.
29. At each step, extend golden tests to compare old-script output with new-service output.
30. If any golden test differs, do not replace the old script.
31. Keep old scripts as reference implementations until migration is stable.
