...
TOOL_LAYOUT: dict[str, tuple[str, int, str]] = {
    "market_position_prompt": ("基金经理", 10, "需要把 Market Basis 更新到最新完整交易日时复制给 Codex。"),
    "strategy_briefing_prompt": ("基金经理", 20, "需要盘前策略简报时复制给 Codex；该任务需要研究上下文。"),
    "generate_target_allocation": ("基金经理", 30, "市场仓位或组合结构变化后使用；生成目标仓位并同步盘中规则。"),
    "valuation_update_check": ("基金经理", 40, "盘前、盘中或复盘前使用；检查估值缺失、过期或跨区，只作为数据质量提示。"),
    "staleness_check": ("基金经理", 50, "怀疑当前研究依赖已过期时使用；可同步 staleness 到盘中规则。"),
    "export_shadow": ("基金经理", 60, "需要审计目标仓位 shadow 结果时使用。"),
    "export_candidate_audit": ("基金经理", 70, "需要审计候选目标仓位时使用。"),
    "theme_research_prompt": ("研究员", 10, "需要更新主线研究时复制给 Codex；通常需要数据源和人工判断。"),
    "etf_research_prompt": ("研究员", 20, "需要为单只 ETF 补 ResearchFirst 档案时使用。"),
    "stock_research_prompt": ("研究员", 30, "需要为单只个股补 profile、valuation、liquidity 时使用。"),
    "theme_leaders": ("研究员", 40, "主线研究更新后使用；生成 ETF/个股候选池和 ResearchFirst 清单。"),
    "valuation_reports": ("研究员", 50, "补估值报告或盘中规则参考区间时使用；批量运行可能较慢。"),
    "audit_holdings_research": ("研究员", 60, "持仓变化或补研究前使用；找出 profile、valuation、liquidity 缺口。"),
    "filter_current_backlog": ("研究员", 70, "已有 backlog 但只想看当前真实持仓相关任务时使用。"),
    "p2_position_review": ("研究员", 80, "大持仓需要单独复核时使用；输出比例口径复核。"),
    "qmt_probe": ("操盘手", 10, "QMT 客户端刚打开后先使用；确认只读连接正常。"),
    "qmt_snapshot": ("操盘手", 20, "盘前或持仓变化后使用；先用 QMT 只读刷新组合比例快照。"),
    "generate_action_plan": ("操盘手", 30, "市场仓位、组合快照、估值和 ResearchFirst 前置研究更新后使用。"),
    "generate_premarket_check": ("操盘手", 40, "盘前已有操作建议后使用；生成当天执行检查和盘中监控清单。"),
    "intraday_once": ("操盘手", 50, "盘中使用；QMT 在线时按已固化规则做一次触发检查。"),
    "intraday_offline_once": ("操盘手", 60, "盘中或演示时使用；QMT 不在线时用参考价检查规则是否可读。"),
    "generate_post_market_review": ("操盘手", 70, "收盘后使用；对比盘前计划、盘中提醒和实际执行做日常复盘。"),
    "generate_weekly_review": ("操盘手", 80, "周末或周度复盘时使用；生成周度复盘草案。"),
    "repair_portfolio_snapshot": ("操盘手", 90, "最近组合快照字段质量异常时使用；修复后再刷新 Web DB。"),
    "db_history_rebuild": ("历史库与审计", 10, "研究文件批量变化后使用；在 temp/history_db 重建历史事实库。"),
    "db_history_check": ("历史库与审计", 20, "重建或导入历史库后使用；运行 DB-first 严格质量检查。"),
    "db_latest_index_shadow": ("历史库与审计", 30, "需要确认 DB 覆盖当前 latest_index.modules 时使用。"),
    "db_export_report": ("历史库与审计", 40, "需要导出 ratio-only 历史库审计报告时使用。"),
    "export_history_snapshot": ("历史库与审计", 50, "需要导出历史快照包做复核时使用。"),
    "review_package": ("历史库与审计", 60, "要发给外部模型或人工审阅前使用；生成 current-only 安全包。"),
    "ingest_web_db": ("系统与开发", 10, "每天打开 Web 前、或生成任何新研究文件后使用；让页面读取最新 current state。"),
    "build_latest_index": ("系统与开发", 20, "发现 latest_index 缺失、过期或刚批量生成研究文件后使用。"),
    "project_check_current": ("系统与开发", 30, "开发、提交或同步前使用；快速检查 current-only 项目状态。"),
    "ratio_only_check": ("系统与开发", 40, "生成或修改 action plan 后使用；确认仍符合比例隐私边界。"),
    "research_first_check": ("系统与开发", 50, "生成或修改 action plan 后使用；确认未绕过 ResearchFirst gate。"),
    "allocation_consistency_check": ("系统与开发", 60, "生成 target allocation 或 intraday rules 后使用。"),
    "hidden_unicode_check": ("系统与开发", 70, "提交前或 GitHub 提示 hidden Unicode warning 时使用。"),
    "web_check": ("系统与开发", 80, "日常 Web 变更后使用；检查页面、API、比例隐私和 current-only 边界。"),
    "web_release_check": ("系统与开发", 90, "发布或提交前使用；包含导出审计、Git 范围和完整 Web 验收。"),
}
...
TOOL_DEFINITIONS: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition(
        ...
        kind="script",
        impact="check",
        description="运行日常 Web 冒烟检查，包含 ingest、ratio-only、ResearchFirst、current-only、API 和页面安全扫描。",
        display_command="python scripts/web_check.py",
        args=_py("scripts/web_check.py"),
        timeout_seconds=420,
    ),
    ToolDefinition(
        id="web_release_check",
        title="Web 发布验收",
        category="检查",
        kind="script",
        impact="check",
        description="运行发布前完整验收，包含 pytest、导出审计、Git 范围、隐藏 Unicode 和 Web 安全扫描。",
        display_command="python scripts/web_release_check.py",
        args=_py("scripts/web_release_check.py"),
        timeout_seconds=900,
    ),
    ToolDefinition(
    ...
    ToolDefinition(
        id="db_history_rebuild",
        title="重建历史事实库",
        category="历史库",
        kind="script",
        impact="write_temp",
        description="在 temp/history_db 重建 DB-first 历史事实库，并导入全部 research JSON artifacts。",
        display_command=(
        ...
        args=_py("scripts/db_migrate.py", "--db", "temp/history_db/myinvest_history.sqlite3", "--reset"),
        timeout_seconds=180,
        post_success_steps=(
            (
                "ingest_research_artifacts",
                _py("scripts/db_ingest_research_artifacts.py", "--db", "temp/history_db/myinvest_history.sqlite3", "--all"),
                300,
            ),
            (
                "db_strict_check",
                _py("scripts/project_check.py", "--current-only", "--db", "temp/history_db/myinvest_history.sqlite3", "--db-strict"),
                180,
            ),
        ),
    ),
    ToolDefinition(
        ...
        kind="script",
        impact="check",
        description="检查 DB-first 历史库迁移、覆盖、归一化、隐私扫描和 current-only 一致性。",
        display_command="python scripts/project_check.py --current-only --db temp/history_db/myinvest_history.sqlite3 --db-strict",
        args=_py("scripts/project_check.py", "--current-only", "--db", "temp/history_db/myinvest_history.sqlite3", "--db-strict"),
        timeout_seconds=180,
    ),
    ToolDefinition(
        id="db_latest_index_shadow",
        title="latest_index 影子比对",
        category="历史库",
        kind="script",
        impact="write_temp",
        description="从历史库生成 latest_index shadow，并严格比对当前 latest_index.modules 覆盖。",
        display_command=(
            "python scripts/db_build_latest_index_shadow.py --db temp/history_db/myinvest_history.sqlite3 "
            "--out temp/history_db/latest_index_shadow.json --strict"
        ),
        args=_py(
            ...
            "--out",
            "temp/history_db/latest_index_shadow.json",
            "--strict",
        ),
        timeout_seconds=180,
    ),
    ToolDefinition(
        ...
        kind="script",
        impact="write_temp",
        description="导出 ratio-only 历史库 JSON/Markdown/zip 审计报告到 temp/history_db/exports。",
        display_command=(
            "python scripts/db_export_report.py --db temp/history_db/myinvest_history.sqlite3 "
            "--out-dir temp/history_db/exports --format both --zip --limit 20"
        ),
        args=_py(
