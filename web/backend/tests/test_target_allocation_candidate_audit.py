from __future__ import annotations

import io
import json
import sqlite3
import subprocess
import sys
import zipfile
from contextlib import closing
from pathlib import Path
from typing import Any

from web.backend.app.db import SessionLocal
from web.backend.app.services.ratio_only import RatioOnlyService
from web.backend.app.services.target_allocation_candidate_audit import (
    ZIP_FILES,
    TargetAllocationCandidateAuditService,
)
from web.backend.app.services.target_allocation_promotion import CANDIDATE_EXPORT_DIR


ROOT = Path(__file__).resolve().parents[3]
DB_PATH = ROOT / "temp" / "web_db" / "myinvest.sqlite"
LOCAL_PATH_RE = RatioOnlyService.local_path_re


def conn(web_db: Path) -> sqlite3.Connection:
    assert web_db == DB_PATH
    connection = sqlite3.connect(web_db)
    connection.row_factory = sqlite3.Row
    return connection


def assert_ratio_only(value: Any) -> None:
    RatioOnlyService.assert_safe(value)
    text = json.dumps(value, ensure_ascii=False, sort_keys=True)
    assert not LOCAL_PATH_RE.search(text)


def current_state_snapshot(web_db: Path) -> dict[str, Any]:
    with closing(conn(web_db)) as db:
        modules = [dict(row) for row in db.execute("SELECT module, artifact_id, updated_at FROM current_modules ORDER BY module")]
        current_artifacts = [
            dict(row)
            for row in db.execute("SELECT module, path, is_current FROM artifacts WHERE is_current = 1 ORDER BY module, path")
        ]
        artifact_count = db.execute("SELECT COUNT(*) AS count FROM artifacts").fetchone()["count"]
    return {
        "latest_index": (ROOT / "research" / "latest_index.json").read_bytes(),
        "current_modules": modules,
        "current_artifacts": current_artifacts,
        "artifact_count": artifact_count,
        "allocation_files": sorted(path.name for path in (ROOT / "research" / "allocation").glob("target_allocation_*.json")),
        "action_files": sorted(path.name for path in (ROOT / "research" / "actions").glob("action_plan_*.json")),
    }


def audit_files() -> set[Path]:
    if not CANDIDATE_EXPORT_DIR.exists():
        return set()
    return set(CANDIDATE_EXPORT_DIR.glob("*candidate_audit*"))


def read_zip_payloads(content: bytes) -> dict[str, Any]:
    with zipfile.ZipFile(io.BytesIO(content)) as archive:
        names = set(archive.namelist())
        assert names == ZIP_FILES
        return {name: json.loads(archive.read(name).decode("utf-8")) for name in names}


def run_cli(*args: str) -> dict[str, Any]:
    proc = subprocess.run(
        [sys.executable, "scripts/export_target_allocation_candidate_audit.py", *args],
        cwd=ROOT,
        text=True,
        encoding="utf-8",
        errors="replace",
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        check=True,
    )
    return json.loads(proc.stdout)


def test_candidate_audit_payload_is_safe(web_db):
    with SessionLocal() as session:
        payload = TargetAllocationCandidateAuditService(session).build_audit_payload()
    assert payload["module"] == "target_allocation_candidate_audit_bundle"
    assert payload["export_type"] == "candidate_audit"
    assert payload["export_mode"] == "candidate_simulation"
    assert payload["status"] == "matched"
    assert {"candidate", "compare", "replay_summary", "promotion_mode", "safety", "system_checks", "provenance"} <= set(payload)
    assert payload["compare"]["matched"] is True
    assert payload["compare"]["diffs"] == []
    assert payload["compare"]["unsupported_fields"] == []
    assert payload["replay_summary"]["failed"] == 0
    assert payload["promotion_mode"]["candidate_allowed"] is False
    assert payload["promotion_mode"]["official_allowed"] is False
    assert payload["safety"]["writes_research_files"] is False
    assert payload["safety"]["updates_latest_index"] is False
    assert payload["safety"]["updates_current_modules"] is False
    assert payload["safety"]["generates_action_plan"] is False
    assert_ratio_only(payload)


def test_candidate_audit_json_bytes(web_db):
    with SessionLocal() as session:
        data = json.loads(TargetAllocationCandidateAuditService(session).build_json_bytes().decode("utf-8"))
    assert data["compare"]["matched"] is True
    assert data["compare"]["diffs"] == []
    assert data["compare"]["unsupported_fields"] == []
    assert data["replay_summary"]["failed"] == 0
    assert data["promotion_mode"]["official_allowed"] is False
    assert_ratio_only(data)


def test_candidate_audit_zip_bytes(web_db):
    with SessionLocal() as session:
        content = TargetAllocationCandidateAuditService(session).build_zip_bytes()
    payloads = read_zip_payloads(content)
    assert payloads["compare_result.json"]["matched"] is True
    assert payloads["compare_result.json"]["diffs"] == []
    assert payloads["replay_summary.json"]["failed"] == 0
    assert payloads["promotion_mode.json"]["official_allowed"] is False
    for payload in payloads.values():
        assert_ratio_only(payload)


def test_candidate_audit_cli_dry_run_does_not_write(web_db):
    before = audit_files()
    summary = run_cli("--dry-run")
    after = audit_files()
    assert after == before
    assert summary["matched"] is True
    assert summary["diff_count"] == 0
    assert summary["unsupported_field_count"] == 0
    assert summary["replay_fail_count"] == 0
    assert summary["official_allowed"] is False
    assert summary["output_path"] is None
    assert_ratio_only(summary)


def test_candidate_audit_cli_json_export(web_db):
    before_state = current_state_snapshot(web_db)
    before_files = audit_files()
    summary = run_cli("--format", "json")
    new_files = audit_files() - before_files
    try:
        assert summary["matched"] is True
        assert summary["replay_fail_count"] == 0
        assert str(summary["output_path"]).startswith("temp/candidate_exports/")
        assert "candidate_audit" in Path(str(summary["output_path"])).name
        assert len(new_files) == 1
        output = ROOT / summary["output_path"]
        data = json.loads(output.read_text(encoding="utf-8"))
        assert data["compare"]["matched"] is True
        assert data["replay_summary"]["failed"] == 0
        assert_ratio_only(data)
        assert current_state_snapshot(web_db) == before_state
    finally:
        for path in new_files:
            path.unlink(missing_ok=True)


def test_candidate_audit_cli_zip_export(web_db):
    before_state = current_state_snapshot(web_db)
    before_files = audit_files()
    summary = run_cli("--format", "zip")
    new_files = audit_files() - before_files
    try:
        assert summary["matched"] is True
        assert summary["replay_fail_count"] == 0
        assert str(summary["output_path"]).startswith("temp/candidate_exports/")
        assert "candidate_audit" in Path(str(summary["output_path"])).name
        assert len(new_files) == 1
        output = ROOT / summary["output_path"]
        payloads = read_zip_payloads(output.read_bytes())
        for payload in payloads.values():
            assert_ratio_only(payload)
        assert current_state_snapshot(web_db) == before_state
    finally:
        for path in new_files:
            path.unlink(missing_ok=True)


def test_candidate_audit_api_json(client):
    for path in ["/api/target-allocation/candidate-audit", "/api/target-allocation/candidate-audit?format=json"]:
        response = client.get(path)
        assert response.status_code == 200
        payload = response.json()
        assert payload["ok"] is True
        assert payload["data"]["compare"]["matched"] is True
        assert payload["data"]["compare"]["diffs"] == []
        assert payload["data"]["replay_summary"]["failed"] == 0
        assert_ratio_only(payload)


def test_candidate_audit_api_zip(client):
    response = client.get("/api/target-allocation/candidate-audit?format=zip")
    assert response.status_code == 200
    assert "application/zip" in response.headers["content-type"]
    payloads = read_zip_payloads(response.content)
    for payload in payloads.values():
        assert_ratio_only(payload)


def test_candidate_audit_no_mutation(web_db, client):
    before_state = current_state_snapshot(web_db)
    with SessionLocal() as session:
        service = TargetAllocationCandidateAuditService(session)
        service.build_audit_payload()
        service.build_json_bytes()
        service.build_zip_bytes()
    run_cli("--dry-run")
    assert client.get("/api/target-allocation/candidate-audit?format=json").status_code == 200
    assert client.get("/api/target-allocation/candidate-audit?format=zip").status_code == 200
    assert current_state_snapshot(web_db) == before_state


def test_candidate_audit_current_only_code_paths():
    paths = [
        ROOT / "web" / "backend" / "app" / "services" / "target_allocation_candidate_audit.py",
        ROOT / "scripts" / "export_target_allocation_candidate_audit.py",
        ROOT / "web" / "backend" / "app" / "routers" / "current.py",
    ]
    for path in paths:
        text = path.read_text(encoding="utf-8")
        assert "latest_index.files" not in text
        assert '["files"]' not in text
        assert "['files']" not in text
        assert "target_allocation_2026-" not in text
        assert "market_score_2026-" not in text
        assert "action_plan_2026-" not in text
